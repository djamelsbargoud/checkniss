...
class CleanUrlSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        with open(self.pathfile + 'promoteur_checker.csv', "r", encoding="utf8") as my_file:
            reader = csv.reader(my_file)
            for row in reader:
        ...
        for promoteur in user_promoteur:
                ...
                yield {'EMAIL' : promoteur[0],
                       'TEL_MOBILE' : promoteur[1],
                       'ispromoteur' : promoteur[2],
                       'Manuel ispromoteur' : promoteur[3],
                        'has_store' : promoteur[4],
                       'TEL.' : tel ,
                        }
